...
-spec encode(form(), binary(), ['role' | 'jid' | 'roomnick' | 'request_allow']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"http://jabber.org/protocol/muc#request">>], [<<"muc#role">>], []).
decode(Fs, XMLNSList) -> decode(Fs, XMLNSList, [<<"muc#role">>], []).
