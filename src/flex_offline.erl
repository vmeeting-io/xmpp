...
-spec encode(form(), binary(), ['number_of_messages']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"http://jabber.org/protocol/offline">>], [], []).
...
encode(List, Lang, Required) ->  Fs = [case Opt of {'number_of_messages', Val} -> ['encode_number_of_messages'(Val, Lang, lists:member('number_of_messages', Required))];#xdata_field{} -> [Opt] end || Opt <- List],  FormType = #xdata_field{var = <<"FORM_TYPE">>, type = hidden,                          values = [<<"http://jabber.org/protocol/offline">>]},  [FormType|lists:flatten(Fs)].
...
do_decode([], _, [], Acc) -> Acc.
-spec 'encode_number_of_messages'(non_neg_integer() | undefined, binary(), boolean()) -> xdata_field().
'encode_number_of_messages'(Value, Lang, IsRequired) ->  Values = case Value of  undefined -> [];
  Value -> [enc_int(Value)]
end,  Opts = [],  #xdata_field{var = <<"number_of_messages">>,               values = Values,               required = IsRequired,               type = 'text-single',               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("Number of Offline Messages"))}.
