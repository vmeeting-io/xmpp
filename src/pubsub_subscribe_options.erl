...
-spec encode(form(), binary(), ['deliver' | 'digest' | 'digest_frequency' | 'expire' | 'include_body' | 'show-values' | 'subscription_type' | 'subscription_depth']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"http://jabber.org/protocol/pubsub#subscribe_options">>], [], []).
...
do_decode([], _, [], Acc) -> Acc.
-spec 'encode_deliver'(boolean() | undefined, binary(), boolean()) -> xdata_field().
'encode_deliver'(Value, Lang, IsRequired) ->  Values = case Value of  undefined -> [];
  Value -> [enc_bool(Value)]
end,  Opts = [],  #xdata_field{var = <<"pubsub#deliver">>,               values = Values,               required = IsRequired,               type = boolean,               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("Whether an entity wants to receive or disable notifications"))}.
-spec 'encode_digest'(boolean() | undefined, binary(), boolean()) -> xdata_field().
'encode_digest'(Value, Lang, IsRequired) ->  Values = case Value of  undefined -> [];
  Value -> [enc_bool(Value)]
end,  Opts = [],  #xdata_field{var = <<"pubsub#digest">>,               values = Values,               required = IsRequired,               type = boolean,               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("Whether an entity wants to receive digests (aggregations) of notifications or all notifications individually"))}.
-spec 'encode_digest_frequency'(binary(), binary(), boolean()) -> xdata_field().
'encode_digest_frequency'(Value, Lang, IsRequired) ->  Values = case Value of  <<>> -> [];
  Value -> [Value]
end,  Opts = [],  #xdata_field{var = <<"pubsub#digest_frequency">>,               values = Values,               required = IsRequired,               type = 'text-single',               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("The minimum number of milliseconds between sending any two notification digests"))}.
-spec 'encode_expire'(binary(), binary(), boolean()) -> xdata_field().
'encode_expire'(Value, Lang, IsRequired) ->  Values = case Value of  <<>> -> [];
  Value -> [Value]
end,  Opts = [],  #xdata_field{var = <<"pubsub#expire">>,               values = Values,               required = IsRequired,               type = 'text-single',               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("The DateTime at which a leased subscription will end or has ended"))}.
-spec 'encode_include_body'(boolean() | undefined, binary(), boolean()) -> xdata_field().
'encode_include_body'(Value, Lang, IsRequired) ->  Values = case Value of  undefined -> [];
  Value -> [enc_bool(Value)]
end,  Opts = [],  #xdata_field{var = <<"pubsub#include_body">>,               values = Values,               required = IsRequired,               type = boolean,               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("Whether an entity wants to receive an XMPP message body in addition to the payload format"))}.
-spec 'encode_show-values'(['show-values'()], default | options('show-values'()), binary(), boolean()) -> xdata_field().
'encode_show-values'(Value, Options, Lang, IsRequired) ->  Values = case Value of  [] -> [];
  Value -> [enc_enum(V) || V <- Value]
end,  Opts = if Options == default ->   [#xdata_option{label = xmpp_tr:tr(Lang, ?T("XMPP Show Value of Away")), value = <<"away">>},#xdata_option{label = xmpp_tr:tr(Lang, ?T("XMPP Show Value of Chat")), value = <<"chat">>},#xdata_option{label = xmpp_tr:tr(Lang, ?T("XMPP Show Value of DND (Do Not Disturb)")), value = <<"dnd">>},#xdata_option{label = xmpp_tr:tr(Lang, ?T("Mere Availability in XMPP (No Show Value)")), value = <<"online">>},#xdata_option{label = xmpp_tr:tr(Lang, ?T("XMPP Show Value of XA (Extended Away)")), value = <<"xa">>}];true ->   [#xdata_option{label = xmpp_tr:tr(Lang, L), value = enc_enum(V)}    || {L, V} <- Options]end,  #xdata_field{var = <<"pubsub#show-values">>,               values = Values,               required = IsRequired,               type = 'list-multi',               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("The presence states for which an entity wants to receive notifications"))}.
-spec 'encode_subscription_type'('subscription_type'() | undefined, default | options('subscription_type'()), binary(), boolean()) -> xdata_field().
'encode_subscription_type'(Value, Options, Lang, IsRequired) ->  Values = case Value of  undefined -> [];
...
-spec 'encode_subscription_depth'('subscription_depth'() | undefined, default | options('subscription_depth'()), binary(), boolean()) -> xdata_field().
'encode_subscription_depth'(Value, Options, Lang, IsRequired) ->  Values = case Value of  undefined -> [];
  Value -> [enc_enum(Value)]
