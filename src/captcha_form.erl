...
-spec encode(form(), binary(), ['answers' | 'audio_recog' | 'challenge' | 'from' | 'ocr' | 'picture_q' | 'picture_recog' | 'qa' | 'SHA-256' | 'sid' | 'speech_q' | 'speech_recog' | 'video_q' | 'video_recog' | 'captcha-fallback-text' | 'captcha-fallback-url']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"urn:xmpp:captcha">>], [], []).
