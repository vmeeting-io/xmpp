...
encode(List, Lang, Required) ->
    Fs = [case Opt of
              ...
              {meetingId, Val} ->
                  [encode_meetingId(Val,
                                    Lang,
                                    lists:member(meetingId, Required))];
              {timeremained, Val} ->
                  [encode_timeremained(Val,
                                       Lang,
                                       lists:member(timeremained, Required))];
              {userDeviceAccessDisabled, Val} ->
                  [encode_userDeviceAccessDisabled(Val,
                                                   Lang,
                                                   lists:member(userDeviceAccessDisabled,
                                                                Required))];
              {lobbyroom, Val} ->
...
do_decode([#xdata_field{var =
    ...
    erlang:error({?MODULE,
                  {too_many_values,
                   ...
                   XMLNS}});
do_decode([#xdata_field{var =
                            <<"muc#roominfo_userDeviceAccessDisabled">>,
                        values = [Value]}
           | Fs],
          XMLNS, Required, Acc) ->
    try dec_bool(Value) of
        Result ->
            do_decode(Fs,
                      XMLNS,
                      lists:delete(<<"muc#roominfo_userDeviceAccessDisabled">>,
                                   Required),
...
do_decode([#xdata_field{var =
    ...
    try Value of
        Result ->
            do_decode(Fs,
                      XMLNS,
                      lists:delete(<<"muc#roominfo_lobbyroom">>, Required),
                      [{lobbyroom, Result} | Acc])
    catch
        _:_ ->
            erlang:error({?MODULE,
                          {bad_var_value, <<"muc#roominfo_lobbyroom">>, XMLNS}})
    end;
do_decode([#xdata_field{var =
                            <<"muc#roominfo_lobbyroom">>,
                        values = []} =
...
do_decode([#xdata_field{var =
    ...
    try Value of
        Result ->
            do_decode(Fs,
                      XMLNS,
                      lists:delete(<<"muc#roominfo_timer_end_time">>,
                                   Required),
                      [{timer_end_time, Result} | Acc])
    catch
        _:_ ->
            erlang:error({?MODULE,
                          {bad_var_value,
...
-spec encode_meetingId(binary(), binary(),
                       boolean()) -> xdata_field().
...
encode_meetingId(Value, Lang, IsRequired) ->
    ...
    #xdata_field{var = <<"muc#roominfo_meetingId">>,
                 values = Values, required = IsRequired,
                 type = 'text-single', options = Opts, desc = <<>>,
                 label = xmpp_tr:tr(Lang, ?T("Unique meeting ID"))}.

-spec encode_timeremained(binary(), binary(),
                          boolean()) -> xdata_field().

encode_timeremained(Value, Lang, IsRequired) ->
    Values = case Value of
                 <<>> -> [];
                 Value -> [Value]
             end,
    Opts = [],
    #xdata_field{var = <<"muc#roominfo_timeremained">>,
                 values = Values, required = IsRequired,
                 type = 'text-single', options = Opts, desc = <<>>,
                 label = xmpp_tr:tr(Lang, ?T("timeremained"))}.

-spec encode_userDeviceAccessDisabled(boolean() |
                                      undefined,
                                      binary(), boolean()) -> xdata_field().

encode_userDeviceAccessDisabled(Value, Lang,
                                IsRequired) ->
    Values = case Value of
                 undefined -> [];
                 Value -> [enc_bool(Value)]
    ...
    #xdata_field{var =
                 ...
                 options = Opts, desc = <<>>,
                 label =
                     xmpp_tr:tr(Lang, ?T("userDeviceAccessDisabled"))}.

-spec encode_lobbyroom(binary(), binary(),
                       boolean()) -> xdata_field().

encode_lobbyroom(Value, Lang, IsRequired) ->
    Values = case Value of
                 <<>> -> [];
                 Value -> [Value]
             end,
    Opts = [],
    #xdata_field{var = <<"muc#roominfo_lobbyroom">>,
                 values = Values, required = IsRequired,
                 type = 'text-single', options = Opts, desc = <<>>,
...
-spec encode_timer_end_time(binary(), binary(),
                            boolean()) -> xdata_field().

encode_timer_end_time(Value, Lang, IsRequired) ->
    Values = case Value of
                 <<>> -> [];
                 Value -> [Value]
             end,
    Opts = [],
    #xdata_field{var = <<"muc#roominfo_timer_end_time">>,
                 values = Values, required = IsRequired,
                 type = 'text-single', options = Opts, desc = <<>>,
