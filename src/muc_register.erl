...
-spec encode(form(), binary(), ['allow' | 'email' | 'faqentry' | 'first' | 'last' | 'roomnick' | 'url']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"http://jabber.org/protocol/muc#register">>], [<<"muc#register_roomnick">>], []).
decode(Fs, XMLNSList) -> decode(Fs, XMLNSList, [<<"muc#register_roomnick">>], []).
