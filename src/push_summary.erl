...
-spec encode(form(), binary(), ['message-count' | 'pending-subscription-count' | 'last-message-sender' | 'last-message-body']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"urn:xmpp:push:summary">>], [], []).
