...
-spec encode(form(), binary(), ['node']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"http://jabber.org/protocol/pubsub#subscribe_authorization">>], [<<"pubsub#node">>], []).
decode(Fs, XMLNSList) -> decode(Fs, XMLNSList, [<<"pubsub#node">>], []).
...
encode(List, Lang, Required) ->  Fs = [case Opt of {'node', Val} ->  ['encode_node'(Val, default, Lang, lists:member('node', Required))];{'node', Val, Opts} ->  ['encode_node'(Val, Opts, Lang, lists:member('node', Required))];#xdata_field{} -> [Opt] end || Opt <- List],  FormType = #xdata_field{var = <<"FORM_TYPE">>, type = hidden,                          values = [<<"http://jabber.org/protocol/pubsub#subscribe_authorization">>]},  [FormType|lists:flatten(Fs)].
do_decode([#xdata_field{var = <<"pubsub#node">>, values = [Value]}|Fs], XMLNS, Required, Acc) ->  try Value of    Result -> do_decode(Fs, XMLNS, lists:delete(<<"pubsub#node">>, Required), [{'node', Result}|Acc])  catch _:_ ->    erlang:error({?MODULE, {bad_var_value, <<"pubsub#node">>, XMLNS}})  end;do_decode([#xdata_field{var = <<"pubsub#node">>, values = []} = F|Fs],       XMLNS, Required, Acc) ->  do_decode([F#xdata_field{var = <<"pubsub#node">>, values = [<<>>]}|Fs],         XMLNS, Required, Acc);do_decode([#xdata_field{var = <<"pubsub#node">>}|_], XMLNS, _, _) ->  erlang:error({?MODULE, {too_many_values, <<"pubsub#node">>, XMLNS}});do_decode([#xdata_field{var = Var}|Fs], XMLNS, Required, Acc) ->  if Var /= <<"FORM_TYPE">> ->    erlang:error({?MODULE, {unknown_var, Var, XMLNS}});  true ->    do_decode(Fs, XMLNS, Required, Acc)  end;do_decode([], XMLNS, [Var|_], _) ->  erlang:error({?MODULE, {missing_required_var, Var, XMLNS}});
do_decode([], _, [], Acc) -> Acc.
-spec 'encode_node'(binary(), default | options(binary()), binary(), boolean()) -> xdata_field().
'encode_node'(Value, Options, Lang, IsRequired) ->  Values = case Value of  <<>> -> [];
  Value -> [Value]
end,  Opts = if Options == default ->   [];true ->   [#xdata_option{label = xmpp_tr:tr(Lang, L), value = V}    || {L, V} <- Options]end,  #xdata_field{var = <<"pubsub#node">>,               values = Values,               required = IsRequired,               type = 'list-single',               options = Opts,               desc = <<>>,               label = xmpp_tr:tr(Lang, ?T("The NodeID of the relevant node"))}.
