...
-spec encode(form(), binary(), ['contact' | 'creation_date' | 'creator' | 'description' | 'language' | 'num_subscribers' | 'owner' | 'publisher' | 'title' | 'type' | 'max_items' | 'access_model' | 'publish_model']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"http://jabber.org/protocol/pubsub#meta-data">>], [], []).
