...
-spec encode(form(), binary(), ['with' | 'start' | 'end' | 'withtext']) -> [xdata_field()].
...
io_format_error({missing_required_var, Var, Type}) ->
    {<<"Missing required field '~s' of type "
       "'~s'">>,
     [Var, Type]}.
decode(Fs) -> decode(Fs, [<<"urn:xmpp:mam:1">>], [], []).
